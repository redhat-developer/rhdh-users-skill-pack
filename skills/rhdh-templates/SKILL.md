...
- **Surgical edits only.** When the user names one template file and one change, edit only that file. Do not add parameters, steps, or files the user did not request.
...
Skip the intake menu when the user already gave a concrete task — for example a
proposal-only templatize request with an output path, or an `add-step` request
that names the template file, step `id`, `action`, and `input`. Route directly
to the matching reference and execute.

Otherwise **Wait for response before proceeding.**
