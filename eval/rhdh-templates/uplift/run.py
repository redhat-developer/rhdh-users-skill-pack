...
def run(script: Path, arguments: list[str], *, capture: bool = False) -> str:
    result = subprocess.run([sys.executable, str(script), *arguments], capture_output=capture, text=True, check=False)
...
def main() -> int:
    ...
    workspace_output = run(scripts / "workspace.py", ["--config", str(config_path), "--run-id", args.run_id], capture=True)
    workspace = next((line.removeprefix("WORKSPACE: ") for line in workspace_output.splitlines() if line.startswith("WORKSPACE: ")), "")
    if not workspace:
        print("AEH did not report a workspace path", file=sys.stderr)
        return 1
    output = Path("eval/runs/rhdh-templates-uplift") / args.run_id
    execute_args = ["--config", str(config_path), "--workspace", workspace, "--model", args.model, "--output", str(output), "--run-id", args.run_id, "--mlflow-experiment", "rhdh-templates-uplift"]
