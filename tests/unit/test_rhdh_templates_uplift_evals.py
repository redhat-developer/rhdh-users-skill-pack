...
def test_uplift_judge_enforces_confirmation_and_exact_modifications() -> None:
    ...
    assert judges.check_uplift_behavior({"annotations": {"case_kind": "confirmation-gate", "required_proposal_markers": ["owner"]}, "files": {"output/proposal.md": "owner"}, "modified_files": {}})[0]
    assert not judges.check_uplift_behavior({"annotations": {"case_kind": "confirmation-gate", "required_proposal_markers": ["owner"]}, "files": {"output/proposal.md": "owner"}, "modified_files": {"fixture/service/package.json": "changed"}})[0]

def test_baseline_judge_rejects_skill_consultation() -> None:
    judges = importlib.import_module("eval.rhdh-templates.uplift.judges")
    outputs = {"events": [{"type": "assistant", "tools": [{"name": "Read", "input": {"file_path": "skills/rhdh-templates/SKILL.md"}}]}]}
